...
int Garnish::timesGarnished=0;//static class variable has to be initialized
...
//	auto findGarnishForFoodItem = [](const FoodItem& f) {//m_mode and m_spices not captured for use in lambda
	auto findGarnishForFoodItem = [=](const FoodItem& f) {
		std::string theGarnish;
		int index = 0;
/*		for (size_t i = 0; i < m_mode.size(); ++i) {//std::list doesn't have a [] operator overload
			if (f.mode == m_mode[i]) {
...
//	std::list<FoodItem>::const_iterator itr;//const_iterator used will not allow for modification
	std::list<FoodItem>::iterator itr;
...
PicnicBasket& PicnicBasket::operator+=(const FoodItem& foodItem) {
...
/*	std::sort(foodItems.begin(), foodItems.end(), [](FoodItem& c1, FoodItem& c2) {//lists do not have random access iters for use with std::sort
