...
int main(void)
{
	...
	scanf_s("%s", lastName, 30);//lastName by itself is an address
	                            //%s is a placeholder for an array of characters
	printf("Enter the student's id number: ");
	scanf_s("%d", &student_id);
	printf("Enter the student's first mark: ");
	scanf_s("%f", &mark1);
	runningTotal += mark1;// runningTotal = runningTotal + mark1
	...
	scanf_s("%f", &mark3);
	runningTotal += mark3;
	averageMark = runningTotal / numMarks;// double / int
