...
int main() {
	...
	cout<<"WeightedListMain: minPriority:"<<minPriority<<" maxPriority:"<<maxPriority<<endl;

	sched_param scheduler;
	scheduler.sched_priority = maxPriority;
	sched_setscheduler(0, SCHED_FIFO, &scheduler);

        int retVal = setpriority(PRIO_PROCESS, 0, -20);
        int niceValue = getpriority(PRIO_PROCESS, 0);
        cout<<"WeightedListMain: retVal:"<<retVal<<" niceValue:"<<niceValue<<endl;
...
//	}
	...
	//EXPERIMENT: Create a list of 20,000 data elements.
