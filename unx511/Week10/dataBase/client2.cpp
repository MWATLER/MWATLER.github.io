//client2.cpp - A client that adds to and reads from a database. 
//
...
const int PORT2=1162;//Port of client 2
...
void *send_func(void *arg)
{
    ...
    while(is_running) {
        ...
        for(int j=0; j<25; ++j) {
            sem_wait(sem_id);
            for(int i=0; i<25; ++i) {
                len=sprintf(buf, "insert cl2%c%c,data2%c%c", entry2, entry1, entry2, entry1)+1;
