...
static void shutdownHandler(int sig)
{
    switch(sig) {
        ...
        case SIGALRM:
            is_running=false;
	    cout<<"shutdownHandler: SIGALRM is_running=false"<<endl;
            break;
    }
}
...
{ 
    ...
    if(msgid<0) {
        cout<<"client3: "<<strerror(errno)<<endl;
	return -1;
    }
    ...
    while(is_running) {
	    ...
	    cout<<"client3: received from message type 3: "<<recvMsg.buf<<endl;
	}
	    ...
	    cout<<"client3: received from message type 4: "<<recvMsg.buf<<endl;
	}
	sleep(1);
    }
...
void *send_func1(void *arg)
{
	...
	Message sendMsg;
        sendMsg.mtype=1;
	if(messageNo<=10) {
            sprintf(sendMsg.buf, "client 3: message type 1 Message #%d\n", messageNo++);
	} else {
            sprintf(sendMsg.buf, "Quit");
            is_running=false;
	}
...
void *send_func2(void *arg)
{
	...
	Message sendMsg;
        sendMsg.mtype=2;
	if(messageNo<=10) {
            sprintf(sendMsg.buf, "client 3: message type 2 Message #%d\n", messageNo++);
	} else {
            sprintf(sendMsg.buf, "Quit");
            is_running=false;
	}
