...
int main(void) {
	...
	while (player[0]->GetTokens() < 300 && player[1]->GetTokens() < 300 && player[2]->GetTokens() < 300) {
		...
		if (player[0]->GetCard() > player[1]->GetCard() && player[0]->GetCard() > player[2]->GetCard()) {
			*player[0] << *player[1];
			*player[0] << *player[2];
		}
		else if (player[1]->GetCard() > player[0]->GetCard() && player[1]->GetCard() > player[2]->GetCard()) {
			*player[1] << *player[0];
			*player[1] << *player[2];
		}
		else if (player[2]->GetCard() > player[0]->GetCard() && player[2]->GetCard() > player[1]->GetCard()) {
			*player[2] << *player[0];
			*player[2] << *player[1];
		}
		else {
