...
int main(void)
{
	...
	int deathYear;//The year of death
	...
	do {
		printf("Enter the last name of the deceased: ");
		scanf("%s", lastName);//3 marks
		if (lastName[0] == '0') {//End the census if '0' is entered, 2 marks
			endCensus = 1;
		}
		else {
			//Prompt for the birth year and death year
			printf("%s's birth year: ", lastName);//3 marks
			scanf("%d", &birthYear);//2 marks
			printf("%s's year of death: ", lastName);//3 marks
			scanf("%d", &deathYear);//2 marks
			if (birthYear > deathYear || deathYear > 2019) {//Validity test, 3 marks
